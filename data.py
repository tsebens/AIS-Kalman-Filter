...
'''
Generate semi noisy location data based on a mathematical function
'''
...
'''
Generate semi noisy heading and SoG data based on location data
'''
def gen_head_and_SoG_data_from_loc(loc_data, noise_fact=0.1):
    ...
    for curr in loc_data[1:]: # Skip the first location, because we've already stored it in prev
        ...
        print('p:%s, %s\nc:%s, %s\ndv:%s, %s' % (prev[0], prev[1], curr[0], curr[1],
                                                 delta_vector[0], delta_vector[1]))
        plot.axes().quiver(curr[0], curr[1], delta_vector[0], delta_vector[1], color='g')
        plot.pause(.5)
        ...
        heading_noise = np.array([noise(), noise()])
        plot.axes().quiver(curr[0], curr[1], heading_noise[0], heading_noise[1], color='k')
        plot.pause(.5)
        ...
        plot.axes().quiver(curr[0], curr[1], heading[0], heading[1], color='b')
        plot.pause(.5)
...
'''
Generate semi noisy ais data, based on pre-generated location data
'''
...
'''
Generate semi noisy ais data, complete with location, heading, and SoG measurements, based on a mathematical function
'''
